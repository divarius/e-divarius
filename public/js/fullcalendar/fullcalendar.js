		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
                                        if (buttonName == 'calendarHeader') {
                                           e.append($('#calendarHeader'));
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {                                                      
							var icon = options.theme ? smartProperty(options.buttonIcons, buttonName) : null; // why are we using smartProperty here?
                                                        if (buttonName == 'resourceNextWeeks') {
                                                            icon = null;
                                                        }
...
function ResourceView(element, calendar, viewName) {
	...
	function buildSkeleton(maxRowCnt, showNumbers) {
		...
		s =
			...
			"<thead>" +
			"<tr class='fc-first fc-last'><th class='fc-resourceName'><a href='habitaciones/add'>Nueva Hab.</a></th>";
		for (i=0; i<colCnt; i++) {
			s +=
	...
	function updateCells(firstTime) {
		...
		headCells.each(function(i, _cell) {
			...
			if (date.getDay() == 1 && viewName == "resourceNextWeeks") {
                            cell.html(cell.html() + '<p class="refWeek"></p>');
